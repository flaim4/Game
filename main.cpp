...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "3D Модель", NULL, NULL);
    if (window == NULL) {
    ...
    if (!loadOBJ("untitled.obj", vertices, indices)) {
        std::cerr << "Не удалось загрузить модель .obj" << std::endl;
        return -1;
    }
